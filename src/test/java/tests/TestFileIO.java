...
class TestFileIO {
	...
	void testLoadFile() {
		...
		User user1 = new User("jacobbutler@companydomain.com", "Jacob", "Butler", Role.LEVEL_3_ENGINEER, Status.INACTIVE_EMPLOYEE, true);
		User user2 = new User("heatherc@companydomain.com", "Heather", "Cameron", Role.LEVEL_2_ENGINEER, Status.ACTIVE_EMPLOYEE, true);
		User user3 = new User("frankdesilets1@gmail.com", "Frank", "Desilets", Role.LEVEL_2_ENGINEER, Status.ACTIVE_EMPLOYEE, true);
		...
		/*
		 * Each user in the correct account list has its properties compared to its counterpart in the loaded list.
		 * Each property should be equal.
		 */
		for (int i = 0; i < 3; i++) {
	...
	void testWriteOneUser() {
		...
		User user1 = new User("frankdesilets1@gmail.com", "Frank", "Desilets", Role.LEVEL_2_ENGINEER, Status.ACTIVE_EMPLOYEE, true);
		correctAccountList.add(user1);
		
		/*
		 * The user is written to the file and a list then populated with
		 * the contents of the file. Before writing, the file is cleared
		 * of contents.
		 */
		...
		/*
		 * The user in the correct account list has its properties compared to its counterpart in the loaded list.
		 * Each property should be equal.
		 */
	...
	void testWriteMultipleUsers() {
		...
		// the correct users are added to the correct user account list
		User user1 = new User("george@gmail.com", "George", "Washington", Role.SENIOR_MANAGER, Status.ACTIVE_EMPLOYEE, true);
		User user2 = new User("john@gmail.com", "John", "Appleseed", Role.LEVEL_2_ENGINEER, Status.INACTIVE_EMPLOYEE, true);
		...
		/*
		 * The users are written to the file and a list then populated with
		 * the contents of the file. Before writing, the file is cleared
		 * of contents.
		 */
		fileIO.writeFile("testUserList.txt", correctAccountList);
	...
	/*
	 * Tests incorrect format detection with multiple incorrectly formatted 
	 * user accounts in a file.
	 */
	@Test
